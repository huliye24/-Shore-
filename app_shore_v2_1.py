    ...
    st.markdown(
        ...
        * {
        ...
        .landing-page {
            ...
            align-items: center;
            background: linear-gradient(180deg, #E8F8F5 0%, #D5F5E3 40%, #FAFBFC 100%);
            text-align: center;
            padding: 40px;
        }
...
def init_state():
    ...
    if "circle_posts_data" not in st.session_state:
        st.session_state.circle_posts_data = {}
...
def page_landing():
    ...
    # 标题
    st.markdown("<div class='landing-title'>岸</div>", unsafe_allow_html=True)
    
    # Slogan
    st.markdown("<div class='landing-slogan'>不需要变好，只需要坐下</div>", unsafe_allow_html=True)
    
    # 进入按钮
    if st.button("轻轻一点，进入岸边", type="primary", use_container_width=False):
        st.session_state.current_page = "login"
        save_data()
        st.rerun()
    
    # 底部提示
    st.markdown("<div style='margin-top: 40px; font-size: 13px; color: #A0AEC0;'>🌊 一个接住脆弱的空间</div>", unsafe_allow_html=True)
...
def page_login():
    ...
    with col2:
        st.markdown("<div style='text-align: center; font-size: 56px; margin-bottom: 8px;'>🌊</div>", unsafe_allow_html=True)
        st.markdown("<div style='text-align: center; font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>欢迎回来</div>", unsafe_allow_html=True)
        st.markdown("<div style='text-align: center; font-size: 13px; color: #718096; margin-bottom: 24px;'>在岸，找到属于你的安静角落</div>", unsafe_allow_html=True)
    
    # 社交登录
    st.markdown("<div style='display: flex; justify-content: center; gap: 16px; margin-bottom: 24px;'>", unsafe_allow_html=True)
    
    col1, col2, col3, col4, col5 = st.columns([1, 1, 1, 1, 1])
    with col2:
        if st.button("🟢", key="wechat_login"):
            ...
            st.rerun()
    with col3:
        if st.button("📱", key="phone_login"):
            st.info("手机号登录开发中")
    with col4:
        if st.button("✉️", key="email_login"):
            st.info("邮箱登录开发中")
    
    st.markdown("</div>", unsafe_allow_html=True)
    
    # 分割线
    st.markdown("<div style='display: flex; align-items: center; margin: 20px 0; color: #A0AEC0; font-size: 13px;'><div style='flex: 1; height: 1px; background: #E2E8F0; margin-right: 12px;'></div>或使用账号密码<div style='flex: 1; height: 1px; background: #E2E8F0; margin-left: 12px;'></div></div>", unsafe_allow_html=True)
    
    # 账号密码
    username = st.text_input("用户名/邮箱/手机号", placeholder="请输入", label_visibility="collapsed")
    password = st.text_input("密码", type="password", placeholder="密码", label_visibility="collapsed")
...
def page_now():
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>✨ 现在</div>", unsafe_allow_html=True)
    ...
    if st.session_state.get("selected_mood"):
        st.info(f"已选择情绪：{st.session_state.selected_mood}")
...
def page_square():
    ...
    st.markdown(f"<div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>🌊 广场</div>", unsafe_allow_html=True)
    ...
    for post in posts:
        ...
        <div class='post-card'>
            <div style='display: flex; align-items: center; margin-bottom: 12px;'>
                <div style='width: 40px; height: 40px; background: linear-gradient(135deg, #4ECDC4 0%, #7EDDD7 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; margin-right: 12px;'>🌊</div>
                <div>
            ...
            <div style='display: inline-block; padding: 4px 12px; background: rgba(78, 205, 196, 0.12); color: #3DBDB5; border-radius: 12px; font-size: 12px;'>{post.get('mood', '未标注')}</div>
        </div>
        """, unsafe_allow_html=True)
...
def page_tearoom():
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>🍵 茶室</div>", unsafe_allow_html=True)
    ...
    if st.session_state.current_circle:
        ...
        <div class='art-card' style='text-align: center;'>
            <div style='font-size: 64px; margin-bottom: 12px;'>{circle['icon']}</div>
            <div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>{circle['name']}</div>
            ...
            for post in posts:
                ...
                <div class='art-card'>
                    <div style='display: flex; align-items: center; margin-bottom: 8px;'>
                        <div style='font-size: 14px; font-weight: 500; color: #2D3748; margin-right: 8px;'>{post['author']}</div>
                        <div style='font-size: 12px; color: #A0AEC0;'>{post['time']}</div>
                    </div>
                    <div style='font-size: 14px; color: #2D3748; line-height: 1.5;'>{post['text']}</div>
                </div>
                """, unsafe_allow_html=True)
        else:
...
def page_scratch():
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>🎁 刮刮乐</div>", unsafe_allow_html=True)
...
def page_mine():
    ...
    st.markdown("<div style='font-size: 22px; font-weight: 600; color: #2D3748; margin-bottom: 4px;'>🏠 我的岸</div>", unsafe_allow_html=True)
    ...
    with tab_real:
        ...
        if posts:
            ...
            for post in posts[:5]:
                st.markdown(f"""
                <div class='art-card' style='padding: 16px;'>
                    <div style='font-size: 14px; color: #2D3748; margin-bottom: 8px;'>{post['text'][:80]}{'...' if len(post['text']) > 80 else ''}</div>
                    <div style='font-size: 12px; color: #A0AEC0;'>{post['time']} · {post.get('mood', '未标注')}</div>
                </div>
                """, unsafe_allow_html=True)
        else:
    ...
    with tab_settings:
        ...
        if st.session_state.friend_requests:
            st.markdown("<div class='art-card'>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size: 16px; font-weight: 600; margin-bottom: 16px;'>好友申请 ({len(st.session_state.friend_requests)})</div>", unsafe_allow_html=True)
